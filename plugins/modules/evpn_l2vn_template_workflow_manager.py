...
__author__ = ['Vivek Raj, Madhan Sankaranarayanan, Rishita Chowdhary, Akash Bhaskaran, Muthu Rakesh, Abhishek Maheshwari, Archit Soni, A Mohamed Rafeek']

DOCUMENTATION = r"""
---
module: evpn_l2vn_template_workflow_manager
...
options:
  ...
  state:
    ...
    default: merged
  l2vn_type:
    description: Type of L2VN to be created.
    type: str
    choices: [localized, anycast]
    required: true
  l2vns:
    description: List of L2VNs to be created.
    type: list
    elements: dict
    required: true
  devices:
    description: List of devices to which L2VNs
      will be applied.
    type: list
    elements: str in l2vn delete case and dict in l2vn create case
    required: true
  config:
...
- name: Deploy L2VN anycast config to each device
    ...
    cisco.dnac.evpn_l2vn_template_workflow_manager:
    dnac_host: "{{ dnac_host }}"
    dnac_port: "{{ dnac_port }}"
...
- name: Deploy L2VN anycast multicast config to each device
    vars:
    mac_address: "{{ item.mac_address }}"
    rendered_l2vn_anycast_mcast: "{{ lookup('template', './jinja_templates/l2vn_anycast_mcast_cli.j2') }}"
    cisco.dnac.evpn_l2vn_template_workflow_manager:
    dnac_host: "{{ dnac_host }}"
    dnac_port: "{{ dnac_port }}"
    dnac_username: "{{ dnac_username }}"
    ...
    config:
        - configuration_templates:
            ...
            template_name: "evpn_l2vn_anycast_mcast_template_{{ item.ip }}"
            template_content: "{{ rendered_l2vn_anycast_mcast }}"
            version_description: "Auto-generated BGP EVPN L2VN anycast multicast config for {{ item.ip }}"
            language: JINJA
            software_type: "IOS-XE"
            device_types:
            - product_family: Switches and Hubs
...
- name: Deploy L2VN localized config to each device
    vars:
    mac_address: "{{ item.mac_address }}"
    rendered_l2vn_localized: "{{ lookup('template', './jinja_templates/l2vn_localized_cli.j2') }}"
    cisco.dnac.evpn_l2vn_template_workflow_manager:
    dnac_host: "{{ dnac_host }}"
    dnac_port: "{{ dnac_port }}"
    dnac_username: "{{ dnac_username }}"
    ...
    config:
        - configuration_templates:
            project_name: "evpn_l2vn_localized"
            template_name: "evpn_l2vn_localized_template_{{ item.ip }}"
            template_content: "{{ rendered_l2vn_localized }}"
            version_description: "Auto-generated BGP EVPN L2VN localized config for {{ item.ip }}"
            language: JINJA
            software_type: "IOS-XE"
            device_types:
            - product_family: Switches and Hubs
...
RT_PATTERN = re.compile(r'^(\d{1,10}):(\d{1,10})$')
MAC_PATTERN = re.compile(r'(?i)^[0-9A-F]{2}(:[0-9A-F]{2}){5}$')
...
class Template(NetworkProfileFunctions):
    ...
    def validate_input(self):
        ...
        # Perform EVPN L2VN specific validations
        if self.params.get("l2vn_type") == "anycast":
            self.validate_evpn_l2vn_anycast_inputs().check_return_status()
        elif self.params.get("l2vn_type") == "localized":
            self.validate_evpn_l2vn_localized_inputs().check_return_status()

        self.msg = "Successfully validated input"
        self.status = "success"
        return self

    def validate_evpn_l2vn_anycast_inputs(self):
        """Validate optional EVPN Anycast L2VN / device parameters passed to the module.

        These parameters are not required for generic template operations and will only
        be validated if provided. They mirror the assertions previously done in the
        playbook so they can be centrally enforced within the module.

        Expected structure (example):
            l2vns:
              - l2vn_name: L2VN-ANYCAST-1
                vlan_id: 20
                vni: 5020
                associated_l3vn: vrf-1
                import_rt: 65000:100
                export_rt: 65000:100
                l2vn_pool:
                  ipv4_address: 12.12.0.1
                  ipv4_netmask: 255.255.0.0
                  ipv6_address: 2002::1 (optional)
                  ipv6_netmask: 64 (required if ipv6_address present)
                dhcp_relay:                    # optional
                  dhcpv4_relay: { dhcpv4_server_ip: 1.1.1.1, dhcpv4_server_vrf: GLOBAL }
                  dhcpv6_relay: { dhcpv6_server_ip: 2001::1, dhcpv6_server_vrf: GLOBAL }
        ...
        Returns:
            self (status failed on validation errors else success/no-op)
        """

        l2vns = self.params.get("l2vns")
        devices = self.params.get("devices")
        errors = []
        if l2vns is None or not l2vns:
            errors.append("At least one L2VN must be defined")
        if devices is None or not devices:
            errors.append("At least one device must be defined")
        
        self.check_valid_l2vn_names(l2vns, errors)
        self.check_valid_vlan_ids(l2vns, errors)
        self.check_valid_vnis(l2vns, errors)
        self.check_valid_l3vn_names(l2vns, errors)
        self.check_valid_l2vn_pools(l2vns, errors)
        self.check_valid_import_export_rts(l2vns, errors)
        self.check_valid_dhcp_relay_settings(l2vns, errors)
        self.check_valid_multicast_groups(l2vns, errors)
        self.check_valid_device_info(devices, errors)

        if errors:
            self.msg = "EVPN anycast input validation failed: " + "; ".join(errors)
            self.status = "failed"
        else:
            self.log("EVPN anycast inputs validated successfully", "INFO")
        return self

    def validate_evpn_l2vn_localized_inputs(self):
        """Validate optional EVPN L2VN localized / device parameters passed to the module.
        ...
        Expected structure (example):
            l2vns:
              - l2vn_name: L2VN-LOC-1
                vlan_id: 60
                associated_l3vn: vrf-1
                l2vn_pool:
                    ipv4_address: 16.16.0.1
                    ipv4_netmask: 255.255.0.0
                    ipv6_address: 2006::1
                    ipv6_netmask: 64
              - l2vn_name: L2VN-LOC-2
                vlan_id: 70
                associated_l3vn: vrf-1
                l2vn_pool:
                    ipv4_address: 17.17.0.1
                    ipv4_netmask: 255.255.0.0
                    ipv6_address: 2007::1
                    ipv6_netmask: 64
            devices:
        ...
        if errors:
            self.msg = "EVPN localized input validation failed: " + "; ".join(errors)
            self.status = "failed"
        else:
            self.log("EVPN localized inputs validated successfully", "INFO")
        return self

    def check_valid_l2vn_names(self, l2vns, errors):
        """Ensure each L2VN has a unique, non-empty, pattern-compliant name.

        Parameters:
            l2vns (list[dict]): L2VN definitions each containing key ``l2vn_name``.

        Adds errors for:
            - Blank names
            - Names not matching ``NAME_PATTERN`` (allowed: alphanum, _-, length ≤ 10)
            - Duplicate names
        ...
        seen = set()
        for l2vn in l2vns:
            name = l2vn.get("l2vn_name")
            if not name or not name.strip():
                errors.append("L2VN name is blank")
            elif not NAME_PATTERN.match(name):
                errors.append(f"L2VN name '{name}' invalid (allowed: alphanum, _-)")
            if name in seen:
                errors.append(f"Duplicate L2VN name '{name}'")
            seen.add(name)


    def check_valid_vlan_ids(self, l2vns, errors):
        """Validate VLAN IDs for presence, numeric range, and uniqueness.

        Parameters:
            l2vns (list[dict]): L2VN definitions each containing key ``vlan_id``.

        Adds errors for:
            - Missing VLAN ID
            - VLAN ID out of allowed range (1–4094)
            - Duplicate VLAN IDs
        ...
        for l2vn in l2vns:
            name = l2vn.get("l2vn_name")
            vlan_id = l2vn.get("vlan_id")
            if vlan_id is None:
                errors.append(f"VLAN ID missing for L2VN '{name}'")
            elif not (1 <= vlan_id <= 4094):
                errors.append(f"VLAN ID {vlan_id} invalid (must be 1-4094)")
            if vlan_id in seen:
                errors.append(f"Duplicate VLAN ID {vlan_id}")
            seen.add(vlan_id)


    def check_valid_vnis(self, l2vns, errors):
        """Validate VNIs for presence, numeric range, and uniqueness.

        Parameters:
            l2vns (list[dict]): L2VN definitions each containing key ``vni``.

        Adds errors for:
            - Missing VNI
            - VNI out of allowed range (1-16777214)
            - Duplicate VNIs
        ...
        for l2vn in l2vns:
            if "vni" in l2vn.keys():
                ...
                vni = l2vn.get("vni")
                if vni is None:
                    errors.append(f"VNI missing for L2VN '{name}'")
                elif not (1 <= vni <= 16777214):
                    errors.append(f"VNI {vni} invalid (must be 1-16777214)")
                if vni in seen:
                    errors.append(f"Duplicate VNI {vni}")
                seen.add(vni)


    def check_valid_l3vn_names(self, l2vns, errors):
        """Validate associated L3VN names for non-empty value, pattern compliance, and uniqueness.

        Parameters:
            l2vns (list[dict]): L2VN definitions containing key ``associated_l3vn``.

        Adds errors for:
            - Blank associated L3VN name
            - Pattern violation (same rules as L2VN name)

        Returns:
            None
        """
        for l2vn in l2vns:
            if "associated_l3vn" in l2vn.keys():
                name = l2vn.get("l2vn_name")
                l3vn = l2vn.get("associated_l3vn")
                if not l3vn or not l3vn.strip():
                    errors.append(f"Associated L3VN name is blank for L2VN '{name}'")
                elif not NAME_PATTERN.match(l3vn):
                    errors.append(f"Associated L3VN name '{l3vn}' invalid (allowed: alphanum, _-")


    def check_valid_l2vn_pools(self, l2vns, errors):
        """Validate IPv4/IPv6 addressing inside each L2VN's address pool.

        Parameters:
            l2vns (list[dict]): L2VN definitions containing key ``l2vn_pool`` with nested IP info.

        Expected pool structure keys:
            - ipv4_address (str)
            - ipv4_netmask (str)
            - ipv6_address (str, optional)
            - ipv6_netmask (int, optional when ipv6_address provided)

        Adds errors for:
            - Missing pool object
            - Missing IPv4 address or netmask
            - Invalid IPv4 address or netmask format
            - IPv6 address provided without netmask, or invalid netmask range (1–128)
            - IPv6 netmask provided without IPv6 address

        Returns:
            None
        """
        for l2vn in l2vns:
            if "l2vn_pool" in l2vn.keys():
                name = l2vn.get("l2vn_name")
                pool = l2vn.get("l2vn_pool")
                if not pool:
                    errors.append(f"L2VN pool is mandatory for L2VN '{name}'")
                    continue

                ipv4 = pool.get("ipv4_address")
                mask = pool.get("ipv4_netmask")
                if not ipv4:
                    errors.append(f"IPv4 pool address missing for L2VN '{name}'")
                elif not self.is_valid_ipv4(ipv4):
                    errors.append(f"Invalid IPv4 pool address '{ipv4}' for L2VN '{name}'")
                if not mask:
                    errors.append(f"IPv4 netmask missing for L2VN '{name}'")
                elif not self.is_valid_ipv4(mask):
                    errors.append(f"Invalid IPv4 netmask '{mask}' for L2VN '{name}'")

                ipv6 = pool.get("ipv6_address")
                ipv6_mask = pool.get("ipv6_netmask")
                if ipv6:
                    if not self.is_valid_ipv6(ipv6):
                        errors.append(f"Invalid IPv6 pool address '{ipv6}' for L2VN '{name}'")
                    if ipv6_mask is None:
                        errors.append(f"IPv6 netmask missing while IPv6 address provided for L2VN '{name}'")
                    elif not (1 <= ipv6_mask <= 128):
                        errors.append(f"Invalid IPv6 netmask '{ipv6_mask}' (1-128) for L2VN '{name}'")
                elif ipv6_mask is not None:
                    errors.append(f"IPv6 netmask provided without IPv6 address for L2VN '{name}'")


    def check_valid_import_export_rts(self, l2vns, errors):
        """Validate import and export route targets (format and numeric ranges).

        Parameters:
            l2vns (list[dict]): L2VN definitions containing optional ``import_rt`` and ``export_rt``.

        Adds errors for malformed or out-of-range route target values.

        Returns:
            None
        """
        for l2vn in l2vns:
            if "import_rt" in l2vn.keys() and "export_rt" in l2vn.keys():
                name = l2vn.get("l2vn_name")
                self.validate_rt(l2vn.get("import_rt"), "import", name, errors)
                self.validate_rt(l2vn.get("export_rt"), "export", name, errors)

    def check_valid_dhcp_relay_settings(self, l2vns, errors):
        """Validate DHCP relay configuration (IPv4 and IPv6) per L2VN.

        Parameters:
            self: Placeholder first argument (these functions are currently module-level; not using instance state).
            l2vns (list[dict]): L2VN definitions. Each may contain a ``dhcp_relay`` dict and optional IPv6 data in ``l2vn_pool``.
            errors (list[str]): Accumulated error messages list to append to.

        Expected structure within each L2VN:
            dhcp_relay (dict, optional):
                - dhcpv4_relay (dict, optional): {
                      'dhcpv4_server_ip': str,
                      'dhcpv4_server_vrf': str
                  }
                - dhcpv6_relay (dict, optional when IPv6 enabled): {
                      'dhcpv6_server_ip': str,
                      'dhcpv6_server_vrf': str
                  }
            l2vn_pool.ipv6_address enables validation of dhcpv6_relay if present.

        Adds errors for:
            - Only one of server IP / VRF supplied (must be both or neither) for v4 or v6
            - Invalid IPv4 / IPv6 address formats for DHCP server addresses
            - IPv6 relay defined when no IPv6 address exists in pool

        Returns:
            None
        """
        for l2vn in l2vns:
            if "dhcp_relay" in l2vn.keys():
                name = l2vn.get("l2vn_name")
                relay = l2vn.get("dhcp_relay")
                pool = l2vn.get("l2vn_pool", {})
                ...
                # DHCPv4 relay
                dhcpv4 = relay.get("dhcpv4_relay") if isinstance(relay, dict) else None
                if dhcpv4:
                    ip = dhcpv4.get("dhcpv4_server_ip")
                    vrf = dhcpv4.get("dhcpv4_server_vrf")
                    if bool(ip) ^ bool(vrf):
                        errors.append(f"DHCPv4 relay must have BOTH server IP and VRF or neither for L2VN '{name}'")
                    if ip and not self.is_valid_ipv4(ip):
                        errors.append(f"Invalid DHCPv4 server IP '{ip}' for L2VN '{name}'")

                # DHCPv6 relay
                if pool.get("ipv6_address"):
                    dhcpv6 = relay.get("dhcpv6_relay") if isinstance(relay, dict) else None
                    if dhcpv6:
                        ip6 = dhcpv6.get("dhcpv6_server_ip")
                        vrf6 = dhcpv6.get("dhcpv6_server_vrf")
                        if bool(ip6) ^ bool(vrf6):
                            errors.append(f"DHCPv6 relay must have BOTH server IP and VRF or neither for L2VN '{name}'")
                        if ip6 and not self.is_valid_ipv6(ip6):
                            errors.append(f"Invalid DHCPv6 server IP '{ip6}' for L2VN '{name}'")


    def check_valid_multicast_groups(self, l2vns, errors):
        """Validate optional IPv4 multicast group addresses in L2VN definitions.

        Parameters:
            self: Placeholder first argument (function is module-level; no instance state used).
            l2vns (list[dict]): L2VN definitions, each may include key ``multicast_group``.
            errors (list[str]): List to append validation error strings.

        Validation performed when ``multicast_group`` is a non-empty string:
            - Must be a syntactically valid IPv4 address.
            - First octet must fall within 224-239 (multicast address block 224.0.0.0/4).

        Adds errors for:
            - Invalid IPv4 formatting.
            - Address outside the multicast range.
        ...
        for l2vn in l2vns:
            name = l2vn.get("l2vn_name")
            mc = l2vn.get("multicast_group")
            if mc and mc.strip():
                if not self.is_valid_ipv4(mc):
                    errors.append(f"Multicast group '{mc}' invalid IPv4 for L2VN '{name}'")
                    continue
                first_octet = int(mc.split(".")[0])
                if not (224 <= first_octet <= 239):
                    errors.append(f"IP '{mc}' not in multicast range 224.0.0.0 - 239.255.255.255")


    def check_valid_device_info(self, devices, errors):
        """Validate device management IP and MAC addresses.

        Parameters:
            devices (list[dict]): Device definitions containing keys ``ip`` and ``mac_address``.

        Adds errors for:
            - Missing management IP
            - Invalid IPv4 management IP format
            - Missing MAC address
            - Invalid MAC address pattern
        ...
        for dev in devices:
            if isinstance(dev, dict):
                ...
                if not ip:
                    errors.append("Device management IP is mandatory")
                elif not self.is_valid_ipv4(ip):
                    errors.append(f"Invalid IPv4 management IP '{ip}'")
                if not mac:
                    errors.append(f"Device MAC address is mandatory for management IP '{ip}'")
                elif not self.is_valid_mac(mac):
                    errors.append(f"Invalid MAC address '{mac}' for management IP '{ip}'")
            elif isinstance(dev, list):
                for dev_ip in dev:
                    if not self.is_valid_ipv4(dev_ip):
                        errors.append(f"Invalid IPv4 management IP '{dev_ip}'")

    def is_valid_ipv4(self, ip):
        """Return True if ``ip`` string is a valid IPv4 address.

        Parameters:
            ip (str): Candidate IPv4 address string.
    ...
    def is_valid_ipv6(self, ip):
        """Return True if ``ip`` string is a valid IPv6 address.

        Parameters:
            ip (str): Candidate IPv6 address string.
    ...
    def is_valid_mac(self, mac):
        """Return True if ``mac`` matches canonical 6-byte hex format (case-insensitive), e.g. 'AA:BB:CC:DD:EE:FF'.
        ...
        Returns:
            bool: True if valid, False otherwise.
        """
        return bool(mac and MAC_PATTERN.match(mac))


    def validate_rt(self, rt, rt_type, l2vn_name, errors):
        """Validate a route target string in 'X:Y' numeric form and append errors if invalid.

        Parameters:
            rt (str): Route target value (e.g. '65000:100'). May be None/blank (ignored).
            rt_type (str): Context label ('import' or 'export').
            l2vn_name (str): Name of the L2VN for error context.

        Adds errors for:
            - Incorrect format (must match ``RT_PATTERN``)
            - First part out of 0-4294967295 range
            - Second part out of 0-65535 range
        ...
        if not rt or not rt.strip():
            return
        m = RT_PATTERN.match(rt)
        if not m:
            errors.append(f"{rt_type} RT '{rt}' invalid format (expected 4byte:2byte, e.g. 65000:100) for L2VN '{l2vn_name}'")
            return

        first, second = int(m.group(1)), int(m.group(2))
        if not (0 <= first <= 4294967295):
            errors.append(f"{rt_type} RT first value out of 4-byte range (0-4294967295) in '{rt}'")
        if not (0 <= second <= 65535):
            errors.append(f"{rt_type} RT second value out of 2-byte range (0-65535) in '{rt}'")
...
def main():
    ...
    element_spec = {
        ...
        "l2vn_type": {"type": "str", "choices": ["anycast", "localized"], "required": True},
        "l2vns": {"type": "list", "elements": "dict", "required": True},
